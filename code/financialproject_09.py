...
inner join poc101_financialproject.loans c on c.loan_id = p.loan_id where member_id NOT IN (select member_id from bad_data_customer)")
...
ldh_ph_df = spark.sql(
    ...
    INNER JOIN poc101_financialproject.loans_defaulters_delinq_new d ON d.member_id = l.member_id  \
...
fh_ldh_ph_df = spark.sql("select ldef.*, \
   ...
   INNER JOIN poc101_financialproject.loans l ON ldef.member_id = l.member_id \
   INNER JOIN poc101_financialproject.customers_new a ON a.member_id = ldef.member_id where ldef.member_id NOT IN (select member_id from bad_data_customer)") 
