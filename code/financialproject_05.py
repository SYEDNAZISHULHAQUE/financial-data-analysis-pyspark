...
# Spark session configured for multi-user YARN environment
...
# Using temp view for quick SQL-based exploration
...
spark.sql(
    ...
    "from loan_defaulters group by delinq_2yrs order by total desc"
).show(40)

# Explicit schema to avoid inferSchema surprises in production
loan_defaulters_schema = (
    "member_id string, delinq_2yrs float, delinq_amnt float, pub_rec float, "
...
# Normalizing delinquency count for easier aggregations
...
# Members with any delinquency history
...
# Members flagged by public records or recent enquiries
...
# Writing both CSV (debugging) and Parquet (analytics)
...
# Persist enquiry-based risk indicators
...
# Casting risk indicators to integers for consistency across models
loans_def_p_inq_last_6mths_df = (
    ...
    .fillna(0, subset=["pub_rec", "pub_rec_bankruptcies", "inq_last_6mths"])
)

loans_def_p_inq_last_6mths_df.createOrReplaceTempView("loan_defaulters")

# Final feature set for downstream risk analysis
